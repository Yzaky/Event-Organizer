...
<div class="container">
	<div id="modif" name="modif" style="display: none;"> <?php  include('content/register_event'); ?> </div>
	<div class="row">
		...
		$('#new_event_form1').submit(function() {
			...
			$.ajax({
				...
				success: function(msg) {
					if (msg) {
						$('#new_event_form1').parent()
							.prepend('<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>L\'événement a été modifier avec succès</div>');
					}
					$('#loader').addClass('hide');
					$('#new_event_form1 :input').attr('disabled', true);
					$('.close').remove;
					delEvt('<?php echo $sup?>');
